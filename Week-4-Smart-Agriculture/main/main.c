...
#define SOIL_ADC ADC1_CHANNEL_6   // GPIO34
#define LIGHT_ADC ADC1_CHANNEL_7  // GPIO35
...
static void read_sensors(void) {
    // Calibrate these two values on the assembled sensor/probe.
    const int soil_dry = 3000, soil_wet = 1300;
    int soil_raw = adc1_get_raw(SOIL_ADC);
    int light_raw = adc1_get_raw(LIGHT_ADC);
    latest.soil_pct = adc_to_percent(soil_raw, soil_dry, soil_wet);
    latest.light_pct = 100.0f * (float)light_raw / 4095.0f;

    // DHT22 one-wire timing is hardware-specific; use a production DHT driver
    // or component here. This fallback keeps the project buildable and makes
    // the missing sensor value explicit rather than fabricating a temperature.
    latest.temp_c = NAN;
...
static void oled_init(void) {
    ...
    // SSD1306 library/component can be attached here. Keeping the transport
    // initialized makes the pin assignment and hardware contract explicit.
}

static void oled_status_task(void *arg) {
    while (1) {
        ESP_LOGI(TAG, "OLED | soil=%.1f%% temp=%s light=%.1f%% pump=%d auto=%d",
                 latest.soil_pct, isnan(latest.temp_c) ? "N/A" : "OK",
                 latest.light_pct, pump_on, auto_mode);
...
static void irrigation_task(void *arg) {
    while (1) {
        ...
        if (pump_on && (xTaskGetTickCount() - pump_started) > pdMS_TO_TICKS(MAX_PUMP_RUNTIME_MS)) {
            ESP_LOGW(TAG, "Pump timeout safety stop");
            pump_set(false);
        }
...
static void mqtt_publish(void) {
    ...
    snprintf(payload, sizeof(payload),
             ...
             latest.soil_pct, isnan(latest.temp_c) ? "null" : "%.2f", latest.light_pct,
             pump_on, latest.rssi, (unsigned long)(esp_log_timestamp() / 1000));
...
static void mqtt_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data) {
    ...
    switch ((esp_mqtt_event_id_t)event_id) {
        case MQTT_EVENT_CONNECTED: {
            char topic[96];
            snprintf(topic, sizeof(topic), "agriculture/%s/command", DEVICE_ID);
            esp_mqtt_client_subscribe(e->client, topic, 1);
            break;
        }
        case MQTT_EVENT_DATA:
            if (strncmp(e->data, "PUMP_ON", e->data_len) == 0) { auto_mode = false; pump_set(true); }
            else if (strncmp(e->data, "PUMP_OFF", e->data_len) == 0) { auto_mode = false; pump_set(false); }
            else if (strncmp(e->data, "AUTO", e->data_len) == 0) auto_mode = true;
            break;
        default: break;
    }
}
...
static void ota_check(void) {
    // OTA is opt-in: replace OTA_URL and add CA validation before deployment.
...
static void prepare_deep_sleep(void) {
    ...
    esp_wifi_stop();
    ESP_LOGI(TAG, "Entering deep sleep for %d seconds", DEEP_SLEEP_SECONDS);
...
void app_main(void) {
    ...
    gpio_config_t relay = {.pin_bit_mask = 1ULL << RELAY_PIN, .mode = GPIO_MODE_OUTPUT};
    ESP_ERROR_CHECK(gpio_config(&relay));
    pump_set(false);
    adc1_config_width(ADC_WIDTH_BIT_12);
    adc1_config_channel_atten(SOIL_ADC, ADC_ATTEN_DB_11);
    adc1_config_channel_atten(LIGHT_ADC, ADC_ATTEN_DB_11);
    oled_init();
    wifi_start();
    ...
    ota_check();
    // For bench testing, comment this call. Production firmware should sleep
    // only after all required telemetry and pump safety checks complete.
    prepare_deep_sleep();
}
